"""
Experiment 4 (GNN): Meta-Level Generalization Test with GNNRateMatrixPredictor.

Trains on peaked distributions at nodes 0, 1, 2 and tests on nodes 3, 4, 5
(never seen during training). The GNN's equivariance should produce comparable
in-distribution and OOD performance.

Saves results as ex4_generalization_gnn.png.
"""
...
def main():
    ...
    # Training distributions: peaked at nodes 0, 1, 2 only
    train_nodes = [0, 1, 2]
    source_train = []
    ...
    print(f"Training: {len(source_train)} sources (nodes 0,1,2)")
    ...
    def evaluate_test_set(sources, targets):
        results = []
        for mu_s, nu_t in zip(sources, targets):
            ...
            times_e, traj_exact = evolve_distribution(mu_s, rate_fn, (0.0, 0.999), n_steps=100)
    ...
    id_results = evaluate_test_set(test_id_sources, test_id_targets)
    print("Evaluating out-of-distribution test cases...")
    ood_results = evaluate_test_set(test_ood_sources, test_ood_targets)

    # Console output
    print("\n=== Validation Results ===")
    print("\nIn-distribution (nodes 0,1,2):")
    for i, (res, node) in enumerate(zip(id_results, test_id_nodes)):
        print(f"  [{i+1}] node={node}, TV_learned={res['tv_learned']:.4f}, "
              f"TV_exact={res['tv_exact']:.4f}")

    print("\nOut-of-distribution (nodes 3,4,5):")
    ...
    fig.suptitle('Experiment 4 (GNN): Meta-Level Generalization (Cycle N=6)', fontsize=13)
    ...
    # Panel B: Entropy — in-distribution
    ax_B = axes[0, 1]
    ...
    # Panel C: Entropy — out-of-distribution
    ax_C = axes[1, 0]
    ...
    ax_D.bar(x, all_tvs, color=bar_colors)
    ax_D.set_xticks(x)
    ...
    ax_D.legend(handles=[Patch(facecolor='steelblue', label='In-distribution'),
                          Patch(facecolor='salmon', label='OOD')], fontsize=9)
    ax_D.grid(True, alpha=0.3, axis='y')

    plt.tight_layout()
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ex4_generalization_gnn.png')
